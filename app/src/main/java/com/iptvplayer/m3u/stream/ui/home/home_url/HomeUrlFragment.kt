package com.iptvplayer.m3u.stream.ui.home.home_url
...
import com.iptvplayer.m3u.stream.databinding.FragmentHomeUrlBinding
...
@AndroidEntryPoint
class HomeUrlFragment : BaseFragment<FragmentHomeUrlBinding, HomeUrlViewMode>() {
    @Inject
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                homeViewModel.playlists.collect { list ->
                    val newList = list.filter { it.playlist.typePlayList == AppConstants.TYPE_PLAYLIST_URL }
                    if (newList.isEmpty()){
