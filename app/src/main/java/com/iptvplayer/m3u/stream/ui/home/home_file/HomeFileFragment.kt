package com.iptvplayer.m3u.stream.ui.home.home_file
...
import com.iptvplayer.m3u.stream.databinding.FragmentHomeFileBinding
...
@AndroidEntryPoint
class HomeFileFragment : BaseFragment<FragmentHomeFileBinding, HomeFileViewModel>() {
    @Inject
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                homeViewModel.playlists.collect { list ->
                    val newList = list.filter { it.playlist.typePlayList == AppConstants.TYPE_PLAYLIST_FILE }
                    if (newList.isEmpty()){
    ...
    fun setUpAdapter() {
        ...
        childFragmentManager.setFragmentResultListener(
            "edit_playlist_result", viewLifecycleOwner
        ) { requestKey, bundle ->
